...
// Security configuration for aircraft tracking endpoints - requires authentication and organization access
...
// GET /api/organizations/[organizationId]/aircraft-tracking - Get real-time aircraft tracking for all organization planes
export const GET = secureApiRoute(async (
  ...
  try {
    ...
    // Role-based access control - school_admin and instructor can only access their own organization
    if (securityContext.user?.role === 'school_admin' || securityContext.user?.role === 'instructor') {
      // Convert both IDs to strings for comparison
      const userSchoolId = securityContext.user?.organization_id?.toString();
      const requestedOrganizationId = params.organization;
      
      if (userSchoolId !== requestedOrganizationId) {
        console.warn(JSON.stringify({
          level: 'WARN',
          message: 'User attempted to access different organization aircraft tracking',
          auditId: securityContext.auditId,
          userId: securityContext.user?._id,
          userRole: securityContext.user?.role,
        ...
        return NextResponse.json({
          success: false,
          error: 'Access denied: You can only view aircraft tracking for your own organization',
          auditId: securityContext.auditId,
          timestamp: new Date().toISOString()
        }, { status: 403 });
      }
    ...
    if (!planes || planes.length === 0) {
      console.log(JSON.stringify({
        level: 'INFO',
        message: 'No planes found for organization',
        auditId: securityContext.auditId,
        userId: securityContext.user?._id,
        organizationId: params.organization,
        timestamp: new Date().toISOString()
      ...
      return NextResponse.json({
        success: true,
        message: 'No planes found for this organization',
        data: {
