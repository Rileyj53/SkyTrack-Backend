...
// POST /api/organizations/[organizationId]/students/[studentId]/flight-invoices/[invoiceId]/finalize - Finalize a flight invoice
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      // This can be extended when the FlightCharge model is available
        ...
        // Add the charge to the student's ledger (still using organization_id in database for now)
...
async function addChargeToStudentLedger(
  ...
  try {
    ...
    if (!ledger) {
      // Create new ledger if it doesn't exist (still using organization_id in database for now)
