...
// GET /api/organizations/[organizationId]/students/[studentId]/ledger - Get student ledger
export const GET = secureApiRoute(async (
  ...
  // Validate IDs
  if (!mongoose.Types.ObjectId.isValid(params.organizationId) || !mongoose.Types.ObjectId.isValid(params.studentId)) {
    return NextResponse.json({
      error: {
        message: 'Invalid organization ID or student ID format',
  ...
  // Check if student exists and belongs to the organization (still using organization_id in database for now)
  ...
  // Find or create student ledger (still using organization_id in database for now)
  ...
  // If ledger doesn't exist, create a new one (still using organization_id in database for now)
  if (!ledger) {
...
export const POST = secureApiRoute(async (
  ...
  // Check if ledger already exists (still using organization_id in database for now)
  ...
  // Create new student ledger (still using organization_id in database for now)
...
export const PUT = secureApiRoute(async (
  ...
  // Find existing ledger (still using organization_id in database for now)
  const existingLedger = await (StudentLedger as any).findOne({
    student_id: params.studentId,
    organization_id: params.organizationId
  });
...
export const DELETE = secureApiRoute(async (
  ...
  // Find and delete the student ledger (still using organization_id in database for now)
