...
// POST /api/organizations/[organizationId]/students/invite - Invite a new student
export const POST = secureApiRoute(async (
  ...
  // Create new student record without user_id (still using organization_id in database for now)
  ...
  const organizationName = organization?.name || 'Your Flight Training Organization';
  ...
  const emailSubject = `You're Invited to ${organizationName} – Set Up Your Student Account Today`;
  const emailContent = `
  <!DOCTYPE html>
  <html>
  <head>
  ...
  <body style="margin:0; padding:0; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height:1.6; color:#333;">
    <div style="max-width:600px; margin:0 auto; background:#fff;">
      ...
      <div style="background:linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding:40px 20px; text-align:center;">
        ...
        <div style="color:#fff; margin:10px 0 0; font-size:32px; font-weight:600;">${organizationName}</div>
      </div>
